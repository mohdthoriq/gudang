...
import Link from 'next/link' // Ganti dari tanstack
import { usePathname } from 'next/navigation' // Ganti dari tanstack
...
export function NavGroup({ items }: NavGroupProps) {
  ...
  const pathname = usePathname() // Mengambil path aktif di Next.js
...
function SidebarMenuCollapsible({
  ...
  useEffect(() => {
    setIsOpen(checkIsActive(href, item, true))
  }, [href, item])
    ...
    >
      <SidebarMenuItem>
        <CollapsibleTrigger asChild>
          <SidebarMenuButton
            tooltip={item.title}
            className='group-data-[state=open]/collapsible:bg-secondary'
          >
            {item.icon && <item.icon />}
            <span>{item.title}</span>
            {item.badge && <NavBadge>{item.badge}</NavBadge>}
